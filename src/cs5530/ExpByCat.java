...
public class ExpByCat extends InputSystem {
	...
	public void sendQuery(Statement stmt) throws Exception {
		...
		try {
			System.out.println("\nResults:\n");
			System.out.printf("| %1$-15s | %2$-3s | %3$-15s | %4$-15s |\n",
					"Category", "hId", "Name", "Avg Cost/Person");
			System.out.println(String.format("%0" + 61 + "d", 0).replace("0","-"));
			...
			if (results.isBeforeFirst()) {
				...
				for (int i = 0; i < groups.size(); ++i) {
					query += "(SELECT AVG(r.price_per_night) AS avgCost, th.hid,  th.category, th.name "
				...
				if (results.isBeforeFirst()) {	
					while (results.next()) {
						...
						for (int i = 0; i < max; ++i) {
							if (i == 0)
								System.out.printf("| %1$-15s | %2$3d | %3$-15s | %4$15.2f |\n",
										catLines.get(i), results.getInt("hid"), nameLines.get(i), results.getDouble("avgCost"));
							else
								System.out.printf("| %1$-15s | %2$-3s | %3$-15s | %4$-15s |\n",
